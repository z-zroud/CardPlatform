...
bool DXTDpParse::HandleDp(const char* szFileName)
{
	ifstream dpFile;
	...
	CPS_ITEM cpsItem;
	cpsItem.fileName = "F:\\CardPlatform\\bin\\huaxin.txt";
			...
			for (auto encryptDGI : vecDGIs)
			{
				if (dgi == encryptDGI) {
					if (dgi == "8000" || dgi == "8020" || dgi == "8002") {
						dgiValue = DecryptDGI("F2CBD6204CE9DAFEC19245704A9B2680", dgiValue,false);
					}
					else {
						dgiValue = DecryptDGI("F2CBD6204CE9DAFEC19245704A9B2680", dgiValue,true);
					}
