...
bool CRenderEngine::DrawImage(HDC hDC, CPaintManagerUI* pManager, const RECT& rcControl, const RECT& rcPaint, CImageAttribute& image, bool bResize)
{
	...
	if (bResize)
	{
	...
	else {	//若不需要拉伸图片，则将图片粘贴到控件的中间位置
		int nControlWidth = rcControl.right - rcControl.left;
		int nControlHeight = rcControl.bottom - rcControl.top;
		int nImageWidth = image.m_rcSource.right - image.m_rcSource.left;
		int nImageHeight = image.m_rcSource.bottom - image.m_rcSource.top;

		int nPaddingX = 0;
		int nPaddingY = 0;
		if (nControlWidth > nImageWidth)
		{
			nPaddingX = (nControlWidth - nImageWidth) / 2;
			rcDest.left += nPaddingX + rcControl.left;
		...
		{
			nPaddingY = (nControlHeight - nImageHeight) / 2;
			rcDest.top += nPaddingY + rcControl.top;
...
#ifdef _MBCS
			WCHAR* pText = charToWchar((char*)pstrText);
			graphics.MeasureString(pText, -1, &font, rectF, &stringFormat, &bounds);
#elif
...
#ifdef _MBCS
			WCHAR* pText = charToWchar((char*)pstrText);
			graphics.DrawString(pText, -1, &font, rectF, &stringFormat, &brush);
#elif
