...
static bool OnComButtonEvent(void* event)
{
	if (((TEventUI*)event)->Type == UIEVENT_BUTTONDOWN ||
		((TEventUI*)event)->Type == UIEVENT_SETCURSOR ||
		((TEventUI*)event)->Type == UIEVENT_KILLFOCUS) {
		CControlUI* pButton = ((TEventUI*)event)->pSender;		
		if (pButton != NULL) {
			::SetCursor(::LoadCursor(NULL, MAKEINTRESOURCE(IDC_ARROW)));
			CComboUI* pCombo = (CComboUI*)(pButton->GetTag());
			if (pCombo != NULL) pCombo->DoEvent(*(TEventUI*)event);
		}
	}

	return true;	//返回false,该事件终止传递。
}
...
void CComboUI::DoInit()
{
	//if (m_pManager)
	//{
	//	CContainerUI* pRootContainer = static_cast<CContainerUI*>(m_pParent);
	//	
	//	m_button->SetManager(m_pManager, m_pParent, false);
	//	LPCTSTR pDefaultAttributes = m_pManager->GetDefaultAttributeList(DUI_CTR_BUTTON);
	//	if (pDefaultAttributes) {
	//		m_button->ApplyAttributeList(pDefaultAttributes);
	//	}

	//	m_cxyFixed.cx -= 10;	//减去右边按钮长度

	//	//设置combox右边按钮的属性
	//	m_button->SetFloat(true);
	//	m_button->SetFixedHeight(m_cxyFixed.cy);	//与combox高度一致
	//	m_button->SetFixedWidth(10);	//固定10
	//	SIZE pos{ 0,0 };					//控件位置
	//	pos.cx = m_cXY.cx + m_cxyFixed.cx;
	//	pos.cy = m_cXY.cy;
	//	m_button->SetFixedXY(pos);
	//	m_button->SetBkColor(this->GetBkColor());	//控件背景颜色

	//	//设置边框
	//	RECT rc = { 1,1,0,1 };
	//	this->SetBorderSize(rc);
	//	RECT rcBtn = { 0,1,1,1 };
	//	m_button->SetBorderSize(rcBtn);
	//	m_button->SetBorderColor(this->GetBorderColor());	//设置边框颜色

	//	m_button->SetTag((UINT_PTR)this);
	//	m_button->OnEvent += MakeDelegate(&OnComButtonEvent);
	//	//m_button
	//	pRootContainer->Add(m_button);
	//}
}
...
void CComboUI::SetPushedBkColorLeft(DWORD dwBkColor)
{
	m_dwPushedBkColorLeft = dwBkColor;
}

void CComboUI::SetPushedBkColorRight(DWORD dwBkColor)
{
	m_dwPushedBkColorRight = dwBkColor;
}
void CComboUI::SetHotBkColorLeft(DWORD dwBkColor)
{
	m_dwHotBkColorLeft = dwBkColor;
}
void CComboUI::SetHotBkColorRight(DWORD dwBkColor)
{
	m_dwHotBkColorRight = dwBkColor;
}

DWORD CComboUI::GetPushedBkColorLeft()const
{
	return m_dwPushedBkColorLeft;
}

DWORD CComboUI::GetPushedBkColorRight()const
{
	return m_dwPushedBkColorRight;
}

DWORD CComboUI::GetHotBkColorLeft()const
{
	return m_dwHotBkColorLeft;
}
DWORD CComboUI::GetHotBkColorRight()const
{
	return m_dwHotBkColorRight;
}
...
void CComboUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	else if( _tcscmp(pstrName, _T("dropboxsize")) == 0)
	{
		...
		LPTSTR pstr = NULL;
		DWORD clrColor = _tcstoul(pstrValue, &pstr, 16);
		SetPushedBkColorRight(clrColor);		//默认该颜色为下拉框的右边按钮
		m_button->SetPushedBkColor(clrColor);
	}
	else if (_tcscmp(pstrName, _T("hotbkcolorleft")) == 0) {
		if (*pstrValue == _T('#')) pstrValue = ::CharNext(pstrValue);
		LPTSTR pstr = NULL;
		DWORD clrColor = _tcstoul(pstrValue, &pstr, 16);
		SetHotBkColorLeft(clrColor);
		SetPushedBkColorLeft(clrColor);	//默认下拉框左半部分hot和push颜色一致
	}
	else if (_tcscmp(pstrName, _T("hotbkcolorright")) == 0) {
		if (*pstrValue == _T('#')) pstrValue = ::CharNext(pstrValue);
		LPTSTR pstr = NULL;
		DWORD clrColor = _tcstoul(pstrValue, &pstr, 16);
		SetHotBkColorRight(clrColor);
		m_button->SetHotBkColor(clrColor);
	}
...
void CComboUI::PaintStatusImage(HDC hDC)
{
    ...
    else if( (m_uButtonState & UISTATE_PUSHED) != 0 ) 
	{
		...
		else if (m_dwPushedBkColorLeft != 0 || m_dwPushedBkColorRight != 0)
		{
			if (m_dwPushedBkColorRight != 0)
			{
				RECT rc = { 0,0,0,0 };
				rc = m_button->GetPos();
				CRenderEngine::DrawColor(hDC, rc, GetAdjustColor(m_dwPushedBkColorRight));
			}
			if (m_dwPushedBkColorLeft != 0)
			{
				CRenderEngine::DrawColor(hDC, m_rcPaint, GetAdjustColor(m_dwPushedBkColorLeft));
			}
			return;
		}
    }
    else if( (m_uButtonState & UISTATE_HOT) != 0 ) 
	{
		...
		else if (m_dwHotBkColorLeft != 0 || m_dwHotBkColorRight != 0)
		{
			if (m_dwHotBkColorLeft != 0)
			{
				CRenderEngine::DrawColor(hDC, m_rcPaint, GetAdjustColor(m_dwHotBkColorLeft));
			}
			if (m_dwHotBkColorRight != 0)
			{
				RECT rc = { 0,0,0,0 };
				rc = m_button->GetPos();
				CRenderEngine::DrawColor(hDC, rc, GetAdjustColor(m_dwHotBkColorRight));
			}
			return;
		}
    }
