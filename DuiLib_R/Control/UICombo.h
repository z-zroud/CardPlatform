...
public:
	...
	bool GetFillPart()const;			//是否将控件划分为两部分考虑，右边部分可自定义背景图，而不用整个控件贴背景图
	void SetFillPart(bool bFillPart);

	DWORD GetFillPartWidth()const;		//设置右半部分的宽度
...
protected:
	...
	bool m_bFillPart;	//是否填充控件部分位置
	DWORD m_dwFillPartWidth;	//填空控件部分位置的宽度
};
