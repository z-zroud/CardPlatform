...
CComboUI::CComboUI() : m_pWindow(NULL), m_iCurSel(-1), m_uButtonState(0)
{
	...
	m_ListInfo.rcTextPadding.left = 10;	//默认字体距离Combo最左边留白10px
	SetBkColor(0xFFFFFFFF);		//设置默认背景颜色
	SetBorderSize({ 1,1,1,1 });
	m_dwHotBkColor = 0xFFFFFFFF;	//设置鼠标悬浮时背景颜色
	m_bFillPart = true;		//下拉列表默认填充部分控件
	m_dwFillPartWidth = 20;	//默认填充控件部分位置的宽度为20px
}
...
void CComboUI::SetFillPart(bool bFillPart)
{
	m_bFillPart = bFillPart;
}

bool CComboUI::GetFillPart()const
{
	return m_bFillPart;
}

DWORD CComboUI::GetFillPartWidth()const
{
	return m_dwFillPartWidth;
}

void CComboUI::SetFillPartWidth(DWORD dwWidth)
{
	m_dwFillPartWidth = dwWidth;
}
...
void CComboUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	else if (_tcscmp(pstrName, _T("fillpart")) == 0) SetFillPart(_tcscmp(pstrValue, _T("true")) == 0);
	else if (_tcscmp(pstrName, _T("fillpartwidth")) == 0) SetFillPartWidth(_ttoi(pstrValue));
...
void CComboUI::PaintStatusImage(HDC hDC)
{
	...
	if (m_bFillPart)	//计算图片填充的位置
	{
		int nControlWidth = m_rcItem.right - m_rcItem.left;
		if (nControlWidth - m_dwFillPartWidth <= 0)
		{
			m_dwFillPartWidth = nControlWidth;
		}
		rcDest.left = m_rcPaint.left + nControlWidth - m_dwFillPartWidth;
		rcDest.right = m_rcPaint.right;
		rcDest.top = m_rcPaint.top;
		rcDest.bottom = m_rcPaint.bottom;
	}
    ...
    if( (m_uButtonState & UISTATE_DISABLED) != 0 )
	{
		...
		{
			if (m_bFillPart)
				DrawImageEx(hDC, m_disabledImage, rcDest);
			else
    ...
    else if( (m_uButtonState & UISTATE_PUSHED) != 0 ) 
	{
		...
		{
			if (m_bFillPart)
				DrawImageEx(hDC, m_pushedImage, rcDest);
			else
    ...
    else if( (m_uButtonState & UISTATE_HOT) != 0 ) 
	{
		...
		{
			if (m_bFillPart)
				DrawImageEx(hDC, m_hotImage, rcDest);
			else
    ...
    else if( (m_uButtonState & UISTATE_FOCUSED) != 0 )
	{
		...
		{
			if (m_bFillPart)
				DrawImageEx(hDC, m_focusedImage, rcDest);
			else
	...
	if (m_bFillPart)
	{
		DrawImageEx(hDC, m_normalImage, rcDest);
	}
	else {
